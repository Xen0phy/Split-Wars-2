...
void RenderArcDPSDump()
{
    ...
    // -------------------------------------------------------------------------
    ImGui::CollapsingHeader("Plugin Exports (PluginInfo)");
    ...
    // -------------------------------------------------------------------------
    // Last Killing Blow — populated by OnCombatSquad() in entry.cpp
    // -------------------------------------------------------------------------
    if (ImGui::CollapsingHeader("Last Killing Blow"))
    {
        if (!HasKillingBlow)
        {
            ImGui::TextDisabled("No killing blow received yet this session.");
        }
        else
        {
            ImGui::Text("Victim name : %s", LastKillingBlow.DestName[0]
                ? LastKillingBlow.DestName : "(unknown)");
            ImGui::Text("Victim ID   : 0x%llX", (unsigned long long)LastKillingBlow.DestAgent);
            ImGui::Text("Source ID   : 0x%llX", (unsigned long long)LastKillingBlow.SourceAgent);
            ImGui::Text("IFF         : %s", IFFName(LastKillingBlow.IFF));
            ImGui::Text("Time        : %llu ms", (unsigned long long)LastKillingBlow.Time);
        }
    }
    ...
    if (ImGui::CollapsingHeader("Current Target"))
    {
        if (!HasTarget)
            ...
            ImGui::Text("Target ID : 0x%llX", (unsigned long long)LastTargetID);
    }
}
